...
void runTerminalReader(ros::NodeHandle& nh) {
    ...
    ros::Publisher lock_pub = nh.advertise<std_msgs::Int8>("throttle_lock_signal", 10);
    ros::Publisher brake_switch_pub = nh.advertise<std_msgs::Int8>("brake_switch_signal", 10);

    char c;

    std::cout<<"Ignition On = 'i'\n"<<"Ignition Off = 'o'\n"<<"Forward Gear = 'b'\n"<<"Neutral = 'n'\n"<<"Reverse Gear = 'm'\n";
    std::cout<<"Set = '1-5'\n"<<"Throttle = 'w'\n"<<"Brake Pedal = 's'\n"<<"Left Steer = 'a'\n"<<"Right Steer = 'd'\n";
    std::cout<<"Throttle Lock = 'l'\n"<<"Brake Switch = 't'\n"<<"Stop Program = 'q'\n";
    while (ros::ok()) {
        if (read(STDIN_FILENO, &c, 1) == 1) {
            ...
            switch (c) {
                ...
                case 'l':
                    msg.data = 0;
                    lock_pub.publish(msg);
                    ROS_INFO ("Throttle Lock signal transmitted");
                    break;
                
                case 't':
                    msg.data = 0;
                    brake_switch_pub.publish(msg);
                    ROS_INFO ("Brake Switch signal transmitted");
                    break;
